		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				<div class="status_info" class="info">选中0张文件，共0B。</div>';
		            html += '				<div class="btns">';
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				$(self).find(".status_info").html("选中"+num+"张文件，共"+size+"。");
			};
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						setTimeout(function(){
							self.funSetStatusInfo(para.ZYFILES.funReturnNeedFiles());  // 显示统计信息
						},0)
						...
						var funAppendPreviewHtml = function(html){
							...
							if(para.dragDrop){
								...
								$(self).find(".add_upload").before(html);
							}
							// 绑定删除按钮
							funBindDelEvent();
							funBindHoverEvent();
						...
						var funBindDelEvent = function(){
							if($(self).find(".file_del").length>0){
								...
								$(self).find(".file_del").click(function() {
									para.ZYFILES.funDeleteFile(parseInt($(this).attr("data-index")), true);
									return false;	
								});
							}
					...
					onSuccess: function(file, response) {
						...
						if(para.finishDel){
							...
							$(self).find("#uploadList_" + file.index).fadeOut();
							// 重新设置统计栏信息
							self.funSetStatusInfo(para.ZYFILES.funReturnNeedFiles());
						}
					},
				...
				para.ZYFILES = $.extend(ZYFILE(), params);
				para.ZYFILES.init();
			};
			...
			this.addEvent = function(){
				//绑定前解除事件
				$(self).find(".filePicker").off('click');
				$(self).find(".webuploader_pick").off('click');
				$(self).find(".upload_btn").off('click');
				$(self).find("#rapidAddImg").off('click');
				// 如果快捷添加文件按钮存在
				if($(self).find(".filePicker").length > 0){
					// 绑定选择事件
					$(self).find(".filePicker").on("click", function(e){
		            	$(self).find("#fileImage").click();
		            });
				}
	            
				// 绑定继续添加点击事件
				$(self).find(".webuploader_pick").on("click", function(e){
	            	$(self).find("#fileImage").click();
	            });
				
				// 绑定上传点击事件
				$(self).find(".upload_btn").on("click", function(e){
					// 判断当前是否有文件需要上传
					if(para.ZYFILES.funReturnNeedFiles().length > 0){
				...
				if($(self).find("#rapidAddImg").length > 0){
					// 绑定添加点击事件
					$(self).find("#rapidAddImg").on("click", function(e){
						$(self).find("#fileImage").click();
		            });
				}
			};
