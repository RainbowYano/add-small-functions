...
<%--                                         <td>${subject.subjectProjectId}</td> --%>
										...
										<!-- 修改标的金额 -->
										<shiro:hasPermission name="subjectMoneyChange">
                                        <c:if test="${ state ==1}">
                                            <button class="btn btn-info" type="button" onclick="modifyAmount(${subject.subjectId})">修改标的金额</button>
										</c:if>
										</shiro:hasPermission>
<!--                                             该审核是提交后审核按钮 -->
...
<!--                                             满表待审核状态的审核按钮 -->
   ...
   function modifyAmount(subjectId){
	   window.location.href="${basePath}cms/subject/transferJsp.action?subjectId="+subjectId;
   }
